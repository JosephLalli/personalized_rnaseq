...
def check_samplesheet(file_in, file_out):
    ...
    sample,sex,fastq_1,fastq_2,strandedness
    SAMPLE_PE,XX,SAMPLE_PE_RUN1_1.fastq.gz,SAMPLE_PE_RUN1_2.fastq.gz,forward
    SAMPLE_PE,XY,SAMPLE_PE_RUN2_1.fastq.gz,SAMPLE_PE_RUN2_2.fastq.gz,forward
    SAMPLE_SE,XX,SAMPLE_SE_RUN1_1.fastq.gz,,forward

    For an example don't see:
    ...
    with open(file_in, "r", encoding='utf-8-sig') as fin:
        ...
        ## Check header
        MIN_COLS = 3
        HEADER = ["sample", "sex", "fastq_1", "fastq_2", "strandedness"]
        ...
        for line in fin:
            if line.strip():
                ...
                sample, sex, fastq_1, fastq_2, strandedness = lspl[: len(HEADER)]
                if sample.find(" ") != -1:
                    print(
                ...
                if sex:
                    if sex not in sexes:
                        print_error(
                            f"Sex must be one of '{', '.join(strandednesses)}'!",
                            "Line",
                            line,
                        )
                else:
                    print_error(
                        f"Sex has not been specified! Must be one of {', '.join(strandednesses)}.",
                        "Line",
                        line,
                    )
                ...
                if sample and fastq_1 and fastq_2:  ## Paired-end short reads
                    sample_info = ["0", fastq_1, fastq_2, strandedness, sex]
                elif sample and fastq_1 and not fastq_2:  ## Single-end short reads
                    sample_info = ["1", fastq_1, fastq_2, strandedness, sex]
                else:
                    print_error("Invalid combination of columns provided!", "Line", line)

                ## Create sample mapping dictionary = {sample: [[ single_end, sex, fastq_1, fastq_2, strandedness ]]}
    ...
    if len(sample_mapping_dict) > 0:
        ...
        with open(file_out, "w") as fout:
            fout.write(
                ",".join(["sample", "single_end", "fastq_1", "fastq_2", "strandedness", "sex"])
                + "\n"
            )
