...
se.dtu <- SummarizedExperiment(
    assays = list(counts = txi.dtu[["counts"]], abundance = txi.dtu[["abundance"]], length = txi.dtu[["length"]]),
    colData = DataFrame(coldata),
    rowData = rowdata
)
...
if ("infReps" %in% names(txi)) {
    ...
    se.ls.infReps <- SummarizedExperiment(
        assays = list(counts = txi.ls.infReps[["counts"]], abundance = txi.ls.infReps[["abundance"]], length = txi.ls.infReps[["length"]]),
        colData = DataFrame(coldata),
        rowData = rowdata
    )
    gse.ls.infReps <- SummarizedExperiment(
        assays = list(counts = gi.ls.infReps[["counts"]], abundance = gi.ls.infReps[["abundance"]], length = gi.ls.infReps[["length"]]),
        colData = DataFrame(coldata),
        rowData = growdata
    )
}
