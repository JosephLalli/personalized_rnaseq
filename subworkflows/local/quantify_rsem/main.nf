...
workflow QUANTIFY_RSEM {
    ...
    index // channel: [ val(meta), [ rsem_index ] ]
    ...
    //
    RSEM_CALCULATEEXPRESSION ( reads.join(index) )
    ...
    //
    RSEM_MERGE_COUNTS (
        RSEM_CALCULATEEXPRESSION.out.counts_gene.join(RSEM_CALCULATEEXPRESSION.out.counts_transcript)
    )
    ...
    merged_counts_gene       = RSEM_MERGE_COUNTS.out.counts_gene              // channel: [ val(meta),  *.gene_counts.tsv ]
    merged_tpm_gene          = RSEM_MERGE_COUNTS.out.tpm_gene                 // channel: [ val(meta),  *.gene_tpm.tsv ]
    merged_counts_transcript = RSEM_MERGE_COUNTS.out.counts_transcript        // channel: [ val(meta),  *.transcript_counts.tsv ]
    merged_tpm_transcript    = RSEM_MERGE_COUNTS.out.tpm_transcript           // channel: [ val(meta),  *.transcript_tpm.tsv ]
