...
include { CONVERT as G2GTOOLS_CONVERT_JUNC } from '../../modules/local/g2gtools/convert/main'
...
workflow QUANTIFY_LEAFCUTTER {
    ...
    //
    junc_files_ch = junc_files.collect{it[1]}
    junc_files_list_ch = junc_files.map({it[1].toString()}).collectFile(name:'input_files.txt', newLine:true)
