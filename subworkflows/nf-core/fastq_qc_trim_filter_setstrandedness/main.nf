...
workflow FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS {
    ...
    if (!skip_bbsplit) {
        BBMAP_BBSPLIT (
            ch_filtered_reads.join(ch_bbsplit_index),
            [],
            [ [], [] ],
            false
        )
    ...
    if (remove_ribo_rna) {
        ...
        if (make_sortmerna_index) {
            SORTMERNA_INDEX (
                ch_sortmerna_fastas.map { meta, fasta -> [[],[], [meta, fasta], [], []] }
            )
            ch_sortmerna_index = SORTMERNA_INDEX.out.index.first()
        }

        SORTMERNA (
            ch_filtered_reads.join(ch_sortmerna_fastas).join(ch_sortmerna_index)
        )
    ...
    // We only run this subworkflow with reference files,
    // before generating personalized references, 
    // so we can assume one unpersonalized reference for this step
    // ch_fasta
    //     .combine(ch_strand_fastq.auto_strand).view{it -> "ch_fasta.combine(ch_strand_fastq.auto_strand): ${it}"}
    //     .map { it.first() }.view{it -> "ch_fasta.combine(ch_strand_fastq.auto_strand).map: ${it}"}
    //     // .first().view{it -> "ch_fasta.combine(ch_strand_fastq.auto_strand).first: ${it}"}
    //     .set { ch_genome_fasta }
    
    // ch_strand_fastq.auto_strand.view{ it -> "ch_strand_fastq.auto_strand: ${it}"}
    // ch_fasta.view{ it -> "ch_genome_fasta: ${it}"}
    // ch_transcript_fasta.view{ it -> "ch_transcript_fasta: ${it}"}
    // ch_gtf.view{ it -> "ch_gtf: ${it}"}
    // ch_salmon_index.view{ it -> "ch_salmon_index: ${it}"}
    ...
    FASTQ_SUBSAMPLE_FQ_SALMON
        ...
        .map {
            meta, json, reads ->
                ...
                // prior code resulted in samples that had different metas depending on whether they had gone through the automatic strandedness pathway
                // salmon_strand_analysis is not used anywhere else, so we can just return the meta
    ...
    multiqc_files   = ch_multiqc_files.transpose().map { it -> it[1] }
