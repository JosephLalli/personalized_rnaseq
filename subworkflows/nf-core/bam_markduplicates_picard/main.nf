...
workflow BAM_MARKDUPLICATES_PICARD {
    ...
    take:
    ch_reads   // channel: [ val(meta), path(reads) ] // reads are in bam/cram/sam format
    ...
    PICARD_MARKDUPLICATES ( ch_reads.join(ch_fasta).join(ch_fai) )
    ...
    ch_reads_index = ch_markdup
        ...
        .join(ch_fasta)
        .map{meta, reads, bai, crai, csi, fasta ->
            if (bai) [ meta, reads, bai, fasta ]
            else if (crai) [ meta, reads, crai, fasta ]
            else [ meta, reads, csi, fasta ]
        }

    BAM_STATS_SAMTOOLS ( ch_reads_index )
