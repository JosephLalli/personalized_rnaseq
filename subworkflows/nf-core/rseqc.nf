...
workflow RSEQC {
    ...
    if ('inner_distance' in rseqc_modules) {
        RSEQC_INNERDISTANCE ( bam.join(bed) )
    ...
    if ('infer_experiment' in rseqc_modules) {
        RSEQC_INFEREXPERIMENT ( bam.join(bed) )
    ...
    if ('junction_annotation' in rseqc_modules) {
        RSEQC_JUNCTIONANNOTATION ( bam.join(bed) )
    ...
    if ('junction_saturation' in rseqc_modules) {
        RSEQC_JUNCTIONSATURATION ( bam.join(bed) )
    ...
    if ('read_distribution' in rseqc_modules) {
        RSEQC_READDISTRIBUTION ( bam.join(bed) )
    ...
    if ('tin' in rseqc_modules) {
        RSEQC_TIN ( bam.join(bai, by: [0]).join(bed) )
