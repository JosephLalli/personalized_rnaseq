...
def getFastpReadsAfterFiltering(json_file, min_num_reads) {
    ...
    def parsed_json = new groovy.json.JsonSlurper().parseText(json_file.text)

    def Map json = parsed_json.get('summary')
...
workflow FASTQ_FASTQC_UMITOOLS_FASTP {
    ...
    if (with_umi && !skip_umi_extract) {
        ...
        if (umi_discard_read in [1,2]) {
            UMITOOLS_EXTRACT
                ...
                .map {
                    meta, read_data ->
                        meta.single_end ? [ meta, read_data ] : [ meta + [single_end: true], read_data[umi_discard_read % 2] ]
                }
                .set { umi_reads }
        }
    }
    ...
    if (!skip_trimming) {
        ...
        FASTP
            ...
            .reads
            .join(trim_json)
            .map { meta, read_data, json -> [ meta, read_data, getFastpReadsAfterFiltering(json, min_trimmed_reads.toLong()) ] }
            .set { ch_num_trimmed_reads }

        ch_num_trimmed_reads
            .filter { _meta, _reads, num_reads -> num_reads >= min_trimmed_reads.toLong() }
            .map { meta, read_data, _num_reads -> [ meta, read_data ] }
            .set { trim_reads }

        ch_num_trimmed_reads
            .map { meta, _read_data, num_reads -> [ meta, num_reads ] }
            .set { trim_read_count }
