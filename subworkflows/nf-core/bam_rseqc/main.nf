...
workflow BAM_RSEQC {
    take:
    bam_bai_bed   // channel: [ val(meta), [ bam, bai , genome.bed] ]
    rseqc_modules //    list: rseqc modules to run

    main:

    bam = bam_bai_bed.map{ it -> [ it[0], it[1] ] }
    ...
    if ('inner_distance' in rseqc_modules) {
        RSEQC_INNERDISTANCE(bam_bai_bed)
    ...
    if ('infer_experiment' in rseqc_modules) {
        RSEQC_INFEREXPERIMENT ( bam_bai_bed )
    ...
    if ('junction_annotation' in rseqc_modules) {
        RSEQC_JUNCTIONANNOTATION ( bam_bai_bed )
    ...
    if ('junction_saturation' in rseqc_modules) {
        RSEQC_JUNCTIONSATURATION ( bam_bai_bed )
    ...
    if ('read_distribution' in rseqc_modules) {
        RSEQC_READDISTRIBUTION ( bam_bai_bed )
