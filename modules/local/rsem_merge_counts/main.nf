process RSEM_MERGE_COUNTS {
    ...
    input:
    tuple val(meta), path ('genes/*'), path ('isoforms/*')

    output:
    tuple val(meta), path ("rsem.merged.gene_counts.tsv" )     , emit: counts_gene
    tuple val(meta), path ("rsem.merged.gene_tpm.tsv")         , emit: tpm_gene
    tuple val(meta), path ("rsem.merged.transcript_counts.tsv"), emit: counts_transcript
    tuple val(meta), path ("rsem.merged.transcript_tpm.tsv" )  , emit: tpm_transcript
