process HISAT2_BUILD {
    ...
    output:
    tuple val(meta), path ("*_hisat2") , emit: index
    ...
    def exon = ''
    def extract_exons = ''
    def hisat2_build_memory = params.hisat2_build_memory ?: "0 GB"
    if (avail_mem >= hisat2_build_memory) {
        log.info "[HISAT2 index build] At least ${hisat2_build_memory} available, so using splice sites and exons to build HISAT2 index"
    ...
    hisat2-build \\
        ...
        $args \\
        $fasta \\
        ${meta.id}_hisat2/${fasta.baseName}
