process BBMAP_BBSPLIT {
    ...
    input:
    tuple val(meta), path(reads), path (primary_ref)
    path  index
    tuple val(other_ref_names), path(other_ref_paths)
    val   only_build_index

    output:
    tuple val(meta), path ("bbsplit")         , optional:true, emit: index
    ...
    def other_refs = []
    other_ref_names.eachWithIndex { name, other_index ->
        other_refs << "ref_${name}=${other_ref_paths[other_index]}"
    }
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    def other_refs = ''
    other_ref_names.eachWithIndex { name, _other_index ->
